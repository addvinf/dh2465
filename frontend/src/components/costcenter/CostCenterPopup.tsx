...
interface CostCenterPopupProps {
  isOpen: boolean;
  searchTerm: string;
  filteredCostCenters: CostCenterSearchResult[];
  selectedCostCenterText?: string;
  ...
  onCostCenterSelect: (costCenter: CostCenterSearchResult) => void;
  onClose: () => void;
}

export function CostCenterPopup({
  isOpen,
  searchTerm,
  filteredCostCenters,
  ...
  const showEmptyState = !loading && !error && filteredCostCenters.length === 0;
  const showResults = !loading && !error && filteredCostCenters.length > 0;

  return (
    <div
      ref={popupRef}
      data-costcenter-popup
        ...
        <div className="flex items-center space-x-2">
          ...
          {searchTerm && (
            <span className="text-xs text-muted-foreground">
              ({filteredCostCenters.length} resultat)
            </span>
          )}
        </div>
      </div>
        ...
        {loading && (
          <div className="p-4 text-center">
            <div className="inline-flex items-center space-x-2 text-muted-foreground">
              ...
              <span className="text-sm">Laddar kostnadsställen...</span>
            </div>
          </div>
        )}
          ...
          <div className="p-4 text-center text-muted-foreground">
            <div className="space-y-1">
              <div className="text-sm">
                {searchTerm
                  ? "Inget kostnadsställe hittades"
                  : "Inga kostnadsställen tillgängliga"}
              </div>
              {searchTerm && (
          ...
          <>
            {filteredCostCenters.map((costCenter, index) => (
              <CostCenterListItem
                ...
                isSelected={costCenter.displayText === selectedCostCenterText}
                isHighlighted={index === highlightedIndex}
                searchTerm={searchTerm}
                onClick={onCostCenterSelect}
              />
            ))}
          </>
        )}
